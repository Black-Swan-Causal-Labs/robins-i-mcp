...
Supplements carry more weight here than in a reporting audit: the target-trial
specification that settles C1-C4, and the analysis detail domains 1 and 4 turn
on, routinely live only in the appendix.
...
def _bytes_to_text(filename: str, data: bytes) -> tuple[str, int | None]:
    """Extract text from an in-memory supplement file, going through a temp file
    only for the formats whose readers need a filesystem path."""
    import tempfile
